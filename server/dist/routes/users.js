...
import { createUserBodySchema, updateUserBodySchema, upsertUserProfileBodySchema, } from '../schemas/users.js';
const userSelect = {
    id: true,
    email: true,
    createdAt: true,
...
usersRouter.post('/users', validateBody(createUserBodySchema), async (req, res) => {
...
usersRouter.put('/users/me', validateBody(updateUserBodySchema), async (req, res) => {
    const authUserId = getAuthUserId(req);
    const body = req.validatedBody;
...
usersRouter.delete('/users/me', async (req, res) => {
    const authUserId = getAuthUserId(req);
    try {
        ...
        if (code === 'P2003' || code === 'P2014') {
            ...
            return errorResponse(res, 400, 'USER_DELETE_CONSTRAINT', 'Cannot delete user with related records');
        }
...
usersRouter.get('/users/:userId/relationship', async (req, res) => {
    const authUserId = getAuthUserId(req);
...
usersRouter.get('/users/me/profile', async (req, res) => {
    const authUserId = getAuthUserId(req);
...
usersRouter.get('/users/:userId/profile', async (req, res) => {
    const authUserId = getAuthUserId(req);
    const userId = req.params['userId'];
...
usersRouter.put('/users/me/profile', validateBody(upsertUserProfileBodySchema), async (req, res) => {
    const authUserId = getAuthUserId(req);
