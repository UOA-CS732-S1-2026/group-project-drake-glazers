...
// ─── Helpers ──────────────────────────────────────────────────────────────────
async function ensureUserOwnsCollection(collectionId, userId) {
...
savedRouter.get('/saved/memories', async (req, res) => {
...
savedRouter.get('/saved/collections', async (req, res) => {
    const userId = getAuthUserId(req);
    ...
    const coverPaths = collections.flatMap((c) => c.items.flatMap((i) => i.memory.media.map((m) => m.mediaPath)));
...
savedRouter.post('/saved/collections', validateBody(createCollectionSchema), async (req, res) => {
    const userId = getAuthUserId(req);
    const { name } = req.body;
...
savedRouter.post('/saved/collections/:id/memories', validateBody(saveMemorySchema), async (req, res) => {
    const userId = getAuthUserId(req);
...
savedRouter.delete('/saved/collections/:id/memories/:memoryId', async (req, res) => {
    const userId = getAuthUserId(req);
    const collectionId = req.params['id'];
...
savedRouter.get('/saved/collections/:id', async (req, res) => {
    const userId = getAuthUserId(req);
    const collectionId = req.params['id'];
    const collection = await ensureUserOwnsCollection(collectionId, userId);
    if (!collection) {
    ...
    const savedItems = (await prisma.savedMemory.findMany({
        where: { collectionId },
        orderBy: { savedAt: 'desc' },
        select: {
            memory: {
...
savedRouter.delete('/saved/collections/:id', async (req, res) => {
    const userId = getAuthUserId(req);
    const collectionId = req.params['id'];
    const collection = await ensureUserOwnsCollection(collectionId, userId);
    if (!collection) {
