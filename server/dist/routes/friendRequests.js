...
friendRequestsRouter.post('/friend-requests', validateBody(sendFriendRequestBodySchema), async (req, res) => {
    const authUserId = getAuthUserId(req);
    const { toUserId } = req.validatedBody;
    if (toUserId === authUserId) {
        return errorResponse(res, 400, 'CANNOT_SEND_TO_SELF', 'Cannot send a friend request to yourself');
    }
    ...
    if (existingRequest) {
        return errorResponse(res, 400, 'FRIEND_REQUEST_ALREADY_EXISTS', 'A pending friend request already exists between these users');
    }
    try {
        ...
        return errorResponse(res, 400, 'FRIEND_REQUEST_CREATE_FAILED', 'Unable to send friend request');
    }
});
...
friendRequestsRouter.get('/friend-requests', async (req, res) => {
    const authUserId = getAuthUserId(req);
...
friendRequestsRouter.put('/friend-requests/:id/accept', async (req, res) => {
    const authUserId = getAuthUserId(req);
    ...
    if (friendRequest.toUserId !== authUserId) {
        return errorResponse(res, 400, 'NOT_RECIPIENT', 'Only the recipient can accept a friend request');
    }
    if (friendRequest.status !== 'pending') {
        return errorResponse(res, 400, 'FRIEND_REQUEST_NOT_PENDING', 'Only pending friend requests can be accepted');
    }
    try {
        ...
        return errorResponse(res, 400, 'FRIEND_REQUEST_ACCEPT_FAILED', 'Unable to accept friend request');
    }
});
// PUT /friend-requests/:id/decline - Decline a friend request (recipient only)
friendRequestsRouter.put('/friend-requests/:id/decline', async (req, res) => {
    const authUserId = getAuthUserId(req);
    const id = req.params['id'];
    const friendRequest = await prisma.friendRequest.findUnique({
        where: { id },
    ...
    if (friendRequest.toUserId !== authUserId) {
        return errorResponse(res, 400, 'NOT_RECIPIENT', 'Only the recipient can decline a friend request');
    }
    if (friendRequest.status !== 'pending') {
        return errorResponse(res, 400, 'FRIEND_REQUEST_NOT_PENDING', 'Only pending friend requests can be declined');
    }
    try {
        ...
        return errorResponse(res, 400, 'FRIEND_REQUEST_DECLINE_FAILED', 'Unable to decline friend request');
    }
});
// DELETE /friend-requests/:id - Cancel a sent friend request (sender only)
friendRequestsRouter.delete('/friend-requests/:id', async (req, res) => {
    const authUserId = getAuthUserId(req);
    const id = req.params['id'];
    const friendRequest = await prisma.friendRequest.findUnique({
        where: { id },
    ...
    if (friendRequest.status !== 'pending') {
        return errorResponse(res, 400, 'FRIEND_REQUEST_NOT_PENDING', 'Only pending friend requests can be cancelled');
    }
    try {
        ...
        return errorResponse(res, 400, 'FRIEND_REQUEST_CANCEL_FAILED', 'Unable to cancel friend request');
    }
});
