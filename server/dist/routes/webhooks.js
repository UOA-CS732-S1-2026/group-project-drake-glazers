...
clerkWebhookRouter.post('/', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    if (!signingSecret) {
        return errorResponse(res, 500, 'WEBHOOK_SIGNING_SECRET_MISSING', 'Clerk webhook signing secret is not configured');
    }
    ...
    try {
        ...
        const normalizedHeaders = Object.fromEntries(Object.entries(req.headers).map(([k, v]) => [
            k,
        ...
        if (!parsedPayload.success) {
            return errorResponse(res, 400, 'INVALID_WEBHOOK_PAYLOAD', 'Clerk webhook payload has an unexpected shape');
        }
        evt = parsedPayload.data;
    }
    catch (err) {
        console.error('Invalid webhook signature:', err);
        return errorResponse(res, 401, 'INVALID_WEBHOOK_SIGNATURE', 'Invalid Clerk webhook signature');
    }
    ...
    try {
        ...
        if (eventType === 'user.created' || eventType === 'user.updated') {
            ...
            if (!email) {
                if (eventType === 'user.created') {
                    return errorResponse(res, 400, 'EMAIL_REQUIRED', 'Clerk webhook user.created event is missing an email address');
                }
        ...
        return errorResponse(res, 500, 'WEBHOOK_PROCESSING_FAILED', 'Unable to process Clerk webhook');
    }
});
