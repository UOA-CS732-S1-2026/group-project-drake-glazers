...
memoriesRouter.get('/explore', async (req, res) => {
    ...
    const paths = memories.map((m) => m.media[0]?.mediaPath).filter(Boolean);
    let signedUrlMap = new Map();
    if (paths.length > 0) {
        const { data: signedUrls } = await supabase.storage
            .from(MEDIA_BUCKET)
            .createSignedUrls(paths, SIGNED_URL_EXPIRY_SECONDS);
        signedUrlMap = new Map((signedUrls ?? [])
...
memoriesRouter.get('/memories', async (req, res) => {
    ...
    const thumbnailPaths = memories.map((m) => m.media[0]?.mediaPath).filter((p) => !!p);
    let signedUrlMap = new Map();
    ...
    return res.status(200).json(memories.map(({ media, ...memory }) => ({
        ...memory,
        thumbnailUrl: media[0] ? (signedUrlMap.get(media[0].mediaPath) ?? null) : null,
    })));
});
memoriesRouter.get('/memories/:id', async (req, res) => {
    const authUserId = getAuthUserId(req);
...
memoriesRouter.put('/memories/:id', validateBody(updateMemoryBodySchema), async (req, res) => {
    const authUserId = getAuthUserId(req);
    const id = req.params.id;
    ...
    if (body.relativeArea !== undefined)
        data.relativeArea = body.relativeArea;
...
memoriesRouter.get('/users/:userId/memories', async (req, res) => {
    const authUserId = getAuthUserId(req);
...
async function attachCoverImages(memories) {
    const coverPaths = memories.map((m) => m.media[0]?.mediaPath).filter((p) => !!p);
    let signedUrlMap = new Map();
...
memoriesRouter.get('/users/:userId/memories/with-covers', async (req, res) => {
    const authUserId = getAuthUserId(req);
    const userId = req.params.userId;
    if (userId === authUserId) {
        const memories = await prisma.memory.findMany({
...
memoriesRouter.post('/memories', validateBody(createMemoryBodySchema), async (req, res) => {
    const authUserId = getAuthUserId(req);
    const body = req.validatedBody;
    try {
